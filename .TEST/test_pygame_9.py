...
class ChessApp:
    ...
    def display_image(self):
        image_path = "images/assets/logo1.png"  # Spécifiez le chemin absolu de votre image
        image = pygame.image.load(image_path)

        # Remplir l'écran avec la couleur bleue
        # self.screen.fill(pygame.Color('blue'))
        self.screen.fill(pygame.Color('#24262f'))  # Remplir l'écran avec la couleur bleue
        ...
        pygame.display.flip()
        pygame.time.delay(5000)  # Mettre en pause pendant 3 secondes avant de démarrer le jeu

        self.screen.fill((0, 0, 0))  # Effacer l'écran après l'affichage de l'image

        pygame.display.flip()  # Mettre à jour l'écran pour effacer l'image précédente
...
class ChessboardWidget:
    def __init__(self):
        ...
        self.border_size = 2
        self.total_size = self.board_size + 3 * self.border_size  # Taille totale avec la bordure
        self.offset_x = (1500 - self.total_size) // 2 + self.border_size  # Nouvelle largeur de la fenêtre
        self.offset_y = (1000 - self.total_size) // 2 + self.border_size  # Nouvelle hauteur de la fenêtre
        self.load_pieces()
    ...
    def draw(self, screen):
        # Dessiner la bordure blanche autour de l'échiquier
        ...
        # Dessiner l'échiquier à l'intérieur de la bordure
        for row in range(8):
            for col in range(8):
