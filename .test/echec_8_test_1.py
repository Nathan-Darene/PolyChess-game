...
class ChessApp:
    ...
    def run(self):
        ...
        running = True
        while running:
            possible_moves = []  # Initialiser la liste des mouvements possibles
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                elif event.type == pygame.VIDEORESIZE:
                    # Gestion du redimensionnement de la fenêtre (non modifié)
                        ...
                        if 0 <= row < 8 and 0 <= col < 8:
                            if self.selected_piece is None:
                                ...
                                if self.selected_piece:
                                    self.help_text = "Coups possibles: " + ", ".join(self.board.get_possible_moves(row, col))
                                else:
                                    self.help_text = "Aucune pièce sélectionnée"
                            else:
            ...
            self.board.draw(self.screen, self.selected_square, possible_moves)  # Passer la liste des mouvements possibles
    ...
    def display_image(self):
        image_path = "images/assets/logo2.png"  # Spécification du chemin de image de l'écran de chargement
...
class ChessboardWidget:
    ...
    def draw(self, screen, selected_square, possible_moves):
            ...
            for col in range(8):
                ...
                if piece_name:
                    ...
                    if (row, col) == selected_square:
                        ...
                        pygame.draw.rect(screen, pygame.Color('green'), rect, 4)  # Afficher les mouvements possibles en vert
    ...
    def move_piece(self, start_square, target_square):
        # Récupérer le nom de la pièce sélectionnée
        piece_name = self.get_piece_name(start_square[0], start_square[1])
        # Afficher le message de déplacement avec le nom de la pièce
        print(f"Déplacement de {piece_name} from {start_square} vers {target_square}")
        return True  # Retourner True pour un déplacement réussi, False sinon
