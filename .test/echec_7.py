...
class ChessboardWidget:
    def __init__(self, font):
        ...
        self.pieces_blanches = {}  # Ajouter l'attribut pour stocker les pièces blanches
        self.pieces_noires = {}  # Ajouter l'attribut pour stocker les pièces noires
        self.load_pieces()
    ...
    def move_piece(self, start_square, target_square):
        # Get the piece name from the start square
        piece_name = self.get_piece_name(start_square[0], start_square[1])

        # Check if the move is valid
        possible_moves = self.get_possible_moves(start_square[0], start_square[1])
        if str(target_square) in possible_moves:
            # Update the position of the piece in the pieces dictionary
            if piece_name:
                if (start_square[0], start_square[1]) in self.pieces_blanches:
                    del self.pieces_blanches[(start_square[0], start_square[1])]
                    self.pieces_blanches[(target_square[0], target_square[1])] = piece_name
                elif (start_square[0], start_square[1]) in self.pieces_noires:
                    del self.pieces_noires[(start_square[0], start_square[1])]
                    self.pieces_noires[(target_square[0], target_square[1])] = piece_name
