...
police = pygame.font.Font(chemin_police1, 50)
police_retoure = pygame.font.Font(chemin_police1, 30)
...
#taille  et police du texte
        ...
        #texte3
        auteur = police_2.render("Nathan Et Sana", True, ROUGE)
        ...
        createur_rect.center = (largeur // 2, 270)
            ...
            if move_made:
                ...
                if game_state.checkmate:
                    ...
                    if game_state.white_to_move:
                        ...
                        text_surface = font.render("Victoire de Noir par ECHEC ET MATE", True, NOIR)
                        ...
                        screen.blit(checkmate_image, checkmate_rect)
                        text_surface = font.render("Victoire de Blanc par ECHEC ET MATE", True, NOIR)
                        text_rect = text_surface.get_rect(center=(self.WIDTH // 2, self.HEIGHT // 2))
                        screen.blit(text_surface, text_rect)
                        pygame.display.flip()
                    ...
                    text_surface = font.render("Stalemate", True, NOIR)
                    text_rect = text_surface.get_rect(center=(self.WIDTH // 2, self.HEIGHT // 2))
                    screen.blit(text_surface, text_rect)
                    pygame.display.flip()
                    print('Stalemate')
...
class EcranChargement:
    ...
    def video_terminee(self):
        ...
        return self.loop_count >= 3


class EcranJeuAI:
    def __init__(self):
        self.message = "La section jouer JcAI est en cours de travaux ..."
        self.message1 = "Merci de votre patience"
        self.police1 = pygame.font.Font(None, 36)  # Chargement  de la police par défaut avec une taille de 36 points
...
class EcranOptions:
    def __init__(self):
        ...
        self.messages = "La section option est en cours de travaux ..."
        self.message1 = "Merci de votre patience"
    ...
    def _afficher_bouton_retour(self):
        ...
        texte_surface = police_retoure.render("Retour", True, BLANC)
        texte_rect = texte_surface.get_rect()
        texte_rect.center = self.bouton_retour.center
        fenetre.blit(texte_surface, texte_rect)


        message_surface = self.police.render(self.messages, True, ROUGE)
        message_surface_1= self.police.render(self.message1, True, NOIR)
        message_rect = message_surface.get_rect()
        message_rect_1 = message_surface_1.get_rect()
        message_rect.center = (largeur // 2, hauteur // 2)
...
class EcranAide:
    def __init__(self):
        self.message = "Aide"
        self.bouton_retour = pygame.Rect(20, 20, 100, 40)
        self.messages = "La section Aide est en cours de travaux ..."
        self.message1 = "Merci de votre patience"
        self.police1 = pygame.font.Font(None, 36)  # Charger une police par défaut avec une taille de 36 points
        self.police = pygame.font.Font("font/font_4/Coffee Normal.ttf", 48)  # Charger une police personnalisée
        self.image = pygame.image.load("images/victoire/138-1384473_clipart-travaux.png")  # Charger votre image
...
if __name__ == "__main__":
    menu = Menu(["Jouer JcJ", "Jouer JcIA", "Options","Aide", "Quitter"])
    # ecran_jeu = EcranJeu()
    ecran_jeu_ai = EcranJeuAI()
    ecran_options = EcranOptions()
    ecran_aide = EcranAide()
    ...
    while True:
        if ecran_actuel == "Menu":
            ...
            if ecran_chargement.video_terminee():
                ecran_jeu_ai.afficher() # Afficher le jeu JeuJcJ lorsque ecran_actuel est "jeu_JcIA"
            else:
                ecran_chargement.afficher()
        elif ecran_actuel == "Options":
            if ecran_chargement.video_terminee():
                ecran_jeu_ai.afficher() # Afficher le jeu JeuJcJ lorsque ecran_actuel est "Options"
            else:
                ecran_options.afficher()

        elif ecran_actuel == "Aide":
            if ecran_chargement.video_terminee():
                ecran_jeu_ai.afficher() # Afficher le jeu JeuJcJ lorsque ecran_actuel est "Aide"
            else:
                ecran_aide.afficher()
